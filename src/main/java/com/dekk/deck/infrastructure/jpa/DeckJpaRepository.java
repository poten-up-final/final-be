...
public interface DeckJpaRepository extends JpaRepository<Deck, Long> {
    ...
    @Query(
            "SELECT d FROM Deck d JOIN DeckMember dm ON d.id = dm.deckId WHERE dm.userId = :userId AND d.deckType = :deckType")
    ...
    @Query(
            "SELECT d FROM Deck d JOIN DeckMember dm ON d.id = dm.deckId WHERE dm.userId = :userId AND d.deckType != :deckType ORDER BY d.createdAt DESC")
    List<Deck> findByUserIdAndDeckTypeNotOrderByCreatedAtDesc(
            @Param("userId") Long userId, @Param("deckType") DeckType deckType);

    @Query(
            "SELECT d FROM Deck d JOIN DeckMember dm ON d.id = dm.deckId WHERE dm.userId = :userId ORDER BY CASE WHEN d.deckType = :defaultType THEN 0 ELSE 1 END, d.createdAt DESC")
