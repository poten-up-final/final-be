...
public class AdminInspectionFallbackServiceImpl implements InspectionFallbackHandler {
    ...
    public void handleFailure(Long productImageId) {
        ...
        inspectionRepository
                .findByProductImageId(productImageId)
                .ifPresentOrElse(
                        inspection -> {
                            inspection.updateAiResult(InspectionStatus.WORKER_ERROR, "AI 서버 통신 실패로 검수 지연");
                            log.info(
                                    "[Admin Fallback] 검수 상태 WORKER_ERROR로 업데이트 완료 - productImageId: {}",
                                    productImageId);
                        },
                        () -> log.warn(
                                "[Admin Fallback] 검수 정보를 찾을 수 없습니다. 상태 업데이트 실패 - productImageId: {}", productImageId));
    }
}
